...
if(!$result){
    throw new Expcetion(mysqli_error($conn));
}
...
if(empty($_SESSION['cart_id'])){
    ...
    if(!$cart_result){
        throw new Exception(mysqli_rror($conn));
    }
    if(mysqli_affected_rows($conn) === 0){
        throw new Exception('data was not added to cart table');
    }
    $cart_id = mysqli_insert_id($conn); //get insert id (make a cart id)
...
if(mysqli_affected_rows($conn) === 0){
    throw new Exception('Did not add items to cart');
}
